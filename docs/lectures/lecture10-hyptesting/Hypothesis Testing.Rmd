...
p.frac <- p/(p+q)
q.frac <- q/(p+q)
sd.p <- sqrt(p.frac*q.frac/(p+q))
z.p <- (p.frac - 0.656)/sd.p
p.val.p <- pnorm(z.p)

p.frac <- round(p.frac, digits=3)
q.frac <- round(q.frac, digits=3)
sd.p <- round(sd.p, digits=3)
...
![King County Views](www/king.county.jpg)
...
![Breakfast Study](www/breakfast.png)
...
* Find $p$ value
  + $SD(\hat{p})=\sqrt{\frac{`r p.frac` `r q.frac`}{`r p+q`}}$
  + $z$ score of difference = $\frac{`r p.frac` - 0.656}{`r sd.p`} = `r z.p`$
