...
------------- ----------------------                     ----------------------  
Yes           $z\times SE=z\frac{\sigma}{\sqrt{n}}$      $z\times SE=z\sqrt{\frac{pq}{n}}$  
...
Yes           $pq$ from null                    If sample is representative, $pq$
              $z\times SE=z\sqrt{\frac{pq}{n}}$ $z\times SE=z\sqrt{\frac{pq}{n}}$
...
* CI: $19.8\pm`r hectares.moe`$
* Is your observed value within the CI?
* Observed: $`r hectares.mean`$, yes within CI
* Cannot reject null
...
* Way 2: Estimate the specific likelihood of observing your mean if we expect the null hypothesis to be true
  ...
  + If $p$ smaller than the critical value, your sample mean is too unlikely $\rightarrow$ reject null
  + If $p$ larger than critical value, your sample mean is within the likely values of  the null; do not reject null

### Example: vineyard size

* Select null: $hectares_{null} = 19.8$
* Set critical value of $t_{df}  = 0.05$
...
* Compare to critical value: 0.025 < `r round(pt(hectares.tdiff, df=(length(vineyards$hectares)-1)), digits=3)` < 0.975 $\rightarrow$ cannot reject null
...
* Practically, $p$ value often more important
    + You can set a critical value but reader also needs to know how unlikely your result is
    + Maybe they think another critical value more important
