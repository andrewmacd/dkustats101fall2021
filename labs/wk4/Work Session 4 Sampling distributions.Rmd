---
title: "Work Session 4 worksheet - sampling distributions"
author: "Student"
date: "9/16/2021"
output:
  html_document:
...
* Learning objectives:
  + Understand how quickly means of proportions converge to theoretical expectations
...
* Make some notes about how the histogram of means changes when you change draws and samples. 
  + How does the shape of the calculated means change as you increase the number of samples? 
  + How does the shape of the calculated means change by increasing the number of draws?
  + How large does number of draws have to be (holding fixed number of samples) for the distribution of means to converge to normal?
  + How large does the number of samples (holding fixed the number of draws) have to be for the distribution of means to converge to normal?

# Interpreting the sampling distribution statistics

* Now calculate the means of these sample distributions
  + How quickly do the means of the sampling distributions converge to the true population mean as $n$ increases (holding constant the number of samples)
  + How quickly do the means of the sampling distributions converge to the true population mean as the number of samples increases? 
...
* Calculate the mean of your `titanic_n_50` distribution sample – is it close to the true population mean?
  + Check the conditions to see if you can rely on a sampling distribution
* Create a 99%, 95%, and a 90% confidence interval for your sample mean
